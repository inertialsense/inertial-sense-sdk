...
void mdns::tick() {
    ...
    if (createMdnsSockets() <= 0) {
        log_warn(IS_LOG_MDNS, "Failed to open any sockets to listen for MDNS responses on");
        return;
    }
...
void mdns::sendQuery(mdns_record_type_t type, const std::string& query) {
    ...
    if (buffer == nullptr) {
        log_error(IS_LOG_MDNS, "Failed to allocate memory for MDNS query are you out of memory?");
        return;
    }
    ...
    for (int isock = 0; isock < socketsOpened; ++isock) {
        if (mdns_query_send(mdnsSockets[isock], type, query.c_str(), strlen(query.c_str()), buffer, capacity, 0)) {
            log_warn(IS_LOG_MDNS, "Failed to send DNS-DS discovery: %s", strerror(errno));
        }
    }
...
int mdns::createMdnsSockets() {
    ...
    if (getifaddrs(&ifaddr) < 0)
        log_warn(IS_LOG_MDNS, "Unable to get interface addresses.");
...
int mdns::queryCallback(int sock, const struct sockaddr* from, size_t addrlen, mdns_entry_type_t entry,
    ...
    if (entry != MDNS_ENTRYTYPE_ANSWER && entry != MDNS_ENTRYTYPE_ADDITIONAL) {
        log_more_debug(IS_LOG_MDNS, "Ignoring AUTHORITY/QUESTION record entry type: %d", entry);
        return 0;
    }
    ...
    if (ttl == 0) {
        ...
        for (const auto& rec : toRemove) {
            responses.erase(rec);
            log_debug(IS_LOG_MDNS, "mDNS goodbye: removed record '%s' type=%d", rec.name.c_str(), rec.type);
        }
        return 0;
    }
    ...
    if (rtype != MDNS_RECORDTYPE_TXT) {
        ...
        if (rtype == MDNS_RECORDTYPE_A) {
            ...
            log_warn(IS_LOG_MDNS, "Unable to process unknown MDNS record type: Not Supported.");
            return -ENOTSUP;
        }
