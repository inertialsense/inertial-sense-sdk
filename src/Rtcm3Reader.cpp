...
cRtcm3Reader::cRtcm3Reader(iRtcm3ReaderDelegate* delegate)
{
	assert(delegate != NULL);
	m_delegate = delegate;
	m_bytesRemaining = 0;
	memset(&m_rtcm, 0, sizeof(m_rtcm));
...
void cRtcm3Reader::WriteByte(uint8_t b)
{
	...
	if (--m_bytesRemaining == 0)
	{
		...
		if (m_rtcm.len > 5)
		{
			...
			uint32_t actualCRC = calculate24BitCRCQ(m_rtcm.buff, m_rtcm.len);

			// get the CRC from the last 3 bytes of the message
			uint32_t correctCRC = getBitsAsUInt32(m_rtcm.buff + m_rtcm.len, 0, 24);

			// if valid CRC, forward mesasge to uINS
			if (actualCRC == correctCRC)
			{
				if (m_delegate->OnPacketReceived(this, m_rtcm.buff, m_rtcm.len))
				{
					ParseMessage();
				}
			}
...
void cRtcm3Reader::ParseMessage()
{
	...
	case DATA_TYPE_OBSERVATION:
		m_delegate->OnObservationReceived(this, &m_rtcm.obs);
		break;
	...
	case DATA_TYPE_ANTENNA_POSITION:
		m_delegate->OnStationReceived(this, &m_rtcm.sta);
		break;
	}
