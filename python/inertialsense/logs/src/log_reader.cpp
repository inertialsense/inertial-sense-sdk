...
void LogReader::organizeData(shared_ptr<cDeviceLog> devLog)
{
    ...
    while ((data = logger_.ReadData(devLog)))
    {
        ...
        switch (data->hdr.id)
        {
            ...
            HANDLE_MSG(DID_IMUX_UNCAL, dev_log_->imu3Uncal);
            HANDLE_MSG(DID_IMUX_RAW, dev_log_->imu3Raw);
...
void LogReader::forwardData(int device_id)
{
    ...
    forward_message(DID_IMUX_UNCAL, dev_log_->imu3Uncal, device_id);
    forward_message(DID_IMUX_RAW, dev_log_->imu3Raw, device_id);
