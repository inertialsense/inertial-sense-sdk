...
void LogReader::log_message(int did, const uint8_t* msg, uint32_t size, std::vector<gps_raw_wrapper_t>& vec) {
    gnss_raw_t tmp{};
    memcpy(&tmp, msg, std::min(sizeof(gnss_raw_t), (size_t)size));
...
void LogReader::forwardData(int device_id) {
    ...
    forward_message(DID_GNSS1_RCVR_POS, dev_log_->gps1UbxPos, device_id);
    ...
    forward_message(DID_GNSS1_RTK_POS, dev_log_->gps1RtkPos, device_id);
