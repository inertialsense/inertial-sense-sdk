...
void GeneratePimu(test_message_t &msg, pimu_t &pimu, int i, float f, bool init=false)
{
    if (init)
    {
...
void GenerateIns1(test_message_t &msg, ins_1_t &ins1, int i, float f, bool init=false)
{
    if (init)
    {
...
void GenerateGpsPos(test_message_t &msg, gps_pos_t &gps, int i, float f, bool init=false)
{
    if (init)
    {
        memset(&gps, 0, sizeof(gps_pos_t));
        gps.timeOfWeekMs = abs(i)*200;
        gps.week = abs(i);
        gps.ecef[0] = f*123.4;
        gps.ecef[1] = f*234.5;
        gps.ecef[2] = f*345.6;
...
void GenerateGpsVel(test_message_t &msg, gps_vel_t &gps, int i, float f, bool init=false)
{
    if (init)
    {
...
void GenerateISB(test_message_t &msg, int i, float f)
{
    ...
    {
        init = false;
        GeneratePimu(  msg, pimu,   i, f, true);
        GenerateIns1(  msg, ins1,   i, f, true);
        GenerateGpsPos(msg, gpsPos, i, f, true);
        GenerateGpsVel(msg, gpsVel, i, f, true);
    }

    switch (i%4)
    {
    default:    GeneratePimu(  msg, pimu,   i, f);   break;
    case 1:     GenerateIns1(  msg, ins1,   i, f);   break;
    case 2:     GenerateGpsPos(msg, gpsPos, i, f);   break;
    case 3:     GenerateGpsVel(msg, gpsVel, i, f);   break;
    }
}

void GenerateUblox(test_message_t &msg, int i, float f)
{
    (void)f;
...
void GenerateRTCM3(test_message_t &msg, int i, float f)
{
    (void)f;
...
// Find next enabled ptype in enum
protocol_type_t iteratePtype()
{
    static protocol_type_t ptype = _PTYPE_NONE;

    do
    {
        ptype = (protocol_type_t)((int)ptype + 1);

        // Restart
        if (ptype > _PTYPE_LAST_DATA)
        { 
            ptype = _PTYPE_FIRST_DATA; 
        }
    }
    while (!ptypeEnabled(ptype));
...
void GenerateMessage(test_message_t &msg, protocol_type_t ptype)
{
    ...
    case _PTYPE_NMEA:
        ...
        case DID_INS_1:     msg.pktSize = nmea_pins1((char*)msg.comm.rxBuf.start, msg.comm.rxBuf.size, msg.data.ins1); break;
        case DID_GPS1_POS:  gpsPos = msg.data.gpsPos; msg.pktSize = nmea_gga((char*)msg.comm.rxBuf.start, msg.comm.rxBuf.size, gpsPos); break;
        case DID_GPS1_VEL:  gpsVel = msg.data.gpsVel; msg.pktSize = nmea_pgpsp((char*)msg.comm.rxBuf.start, msg.comm.rxBuf.size, gpsPos, gpsVel); break;
        }
    ...
    case _PTYPE_UBLOX:
        GenerateUblox(msg, i, f);
        msg.dataHdr.size = msg.comm.rxPkt.dataHdr.size = msg.pktSize;
        msg.comm.rxBuf.tail = msg.comm.rxBuf.start + msg.pktSize;
        break;

    case _PTYPE_RTCM3:
        GenerateRTCM3(msg, i, f);
        msg.dataHdr.size = msg.comm.rxPkt.dataHdr.size = msg.pktSize;
        msg.comm.rxBuf.tail = msg.comm.rxBuf.start + msg.pktSize;
        break;
    }
...
void GenerateLogFiles(int numDevices, string directory, cISLogger::eLogType logType, float logSizeMB, int options)
{
    // Remove old files
    ...
    while (logger.LogSizeMB() < logSizeMB)
    {
        for (int d=0; d<numDevices; d++)
        {
            GenerateMessage(msg, ptype);
