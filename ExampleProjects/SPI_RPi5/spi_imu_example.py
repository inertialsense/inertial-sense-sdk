#!/usr/bin/env python3
"""
InertialSense SPI DID_IMU Example - Raspberry Pi 5

Sends PKT_TYPE_GET_DATA for DID_IMU once per second (Strategy A — fixed-size
polling, no Data Ready pin required), reads a fixed block of SPI bytes each
loop tick, parses any ISB DATA packets that arrive, and prints the imu_t
...
IMU_PERIOD_MS   = 1000      # desired DID_IMU broadcast period in ms (1 Hz)
...
def main() -> None:
    ...
    get_pkt  = build_get_data(DID_IMU, IMU_PERIOD_MS)

    saved_termios = termios.tcgetattr(sys.stdin)
    try:
        tty.setcbreak(sys.stdin.fileno())

        period_actual = math.ceil(IMU_PERIOD_MS / NAV_DT_MS) * NAV_DT_MS
        print(f"InertialSense SPI DID_IMU Example  "
        ...
        print(f"Requested period: ~{period_actual} ms  "
              f"({math.ceil(IMU_PERIOD_MS / NAV_DT_MS)} x {NAV_DT_MS} ms nav DT)")
        ...
        print(f"Sending GET_DATA for DID_IMU @ ~{period_actual} ms ...\n", flush=True)
