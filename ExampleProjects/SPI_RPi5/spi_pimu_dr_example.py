#!/usr/bin/env python3
"""
InertialSense SPI DID_PIMU Example — key-triggered, Data Ready pin
Raspberry Pi 5

Press 'R' to request one DID_PIMU sample (rate-limited to once per 250 ms).
...
DATA packets, and prints the pimu_t fields.  Press 'Q' to quit.
...
Run:
    python3 spi_pimu_dr_example.py
"""
...
REQUEST_MIN_S   = 0.250     # minimum interval between requests (4 Hz max)
...
PKT_TYPE_GET_DATA            = 3       # request a DID (with period)
...
def main() -> None:
    ...
    get_pkt      = build_get_data_once(DID_PIMU)
    ...
    try:
        ...
        print(f"InertialSense SPI DID_PIMU Example  (DR pin, key-triggered)  "
        ...
        print(f"Press '{REQUEST_KEY.upper()}' to request DID_PIMU  |  "
        ...
        while True:
            ...
            if key == REQUEST_KEY:
                ...
                while lgpio.gpio_read(gpio_h, DR_GPIO_LINE) == 0:
                    ...
                    if not found:
                        print(f"  [!] No DID_PIMU packet found in {len(rx_buf)} bytes received")
