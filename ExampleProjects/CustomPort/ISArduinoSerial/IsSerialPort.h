...
private:
    ...
    static const char* name(port_handle_t port) {
        ...
        return ((ISArduinoSerialPort*)port)->m_portName.c_str();
    }
    ...
    static int open(port_handle_t port) {
        ...
        ((ISArduinoSerialPort*)port)->ptype |= PORT_FLAG__OPENED;       // mark the port as opened
    ...
    static int logger(port_handle_t port, uint8_t op, const uint8_t* buf, unsigned int len, void* userData) { return PORT_ERROR__NOT_SUPPORTED; }
