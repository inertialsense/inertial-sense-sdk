 ...
 *
 * In this test, we subscribe to INS_1, GPS_1 and IMU, and also DID_GNSS1_TIMESYNC.  Once we get a GPS fix, we a set of messages over 10 seconds.  Then, we take the average difference between the INS timestamp
 * and the nearest GPS timestamp.  The average deviation should be below <0.5s, but ideally is below 0.1s.
 * DID_GNSS1_TIMESYNC provides the towOffset, timeMcu (effectively time since bootup)
 */
